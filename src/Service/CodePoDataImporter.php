 ...
 *
 * Class CodePoDataImporter
 */
class CodePoDataImporter
{
    const INSERT_TRANSACTION_SIZE = 50;

    const POSTCODE_CSV_INDEX = 0;
    const EASTING_CSV_INDEX  = 2;
    const NORTHING_CSV_INDEX = 3;
    ...
    /**
     * CodePoDataImporter constructor.
     *
     ...
     *
     * @param resource|bool $filePointer
     *
     * @return boolean
     */
    public function csvFileDataImport($filePointer): bool
    {
        if(!$filePointer) {
            return false;
        }
        ...
        {
            $latLon = $this->convertToLatLong($data[self::EASTING_CSV_INDEX], $data[self::NORTHING_CSV_INDEX]);

            $postcode = new Postcode();
            $postcode->setPostcode($data[self::POSTCODE_CSV_INDEX]);
            $postcode->setEasting($data[self::EASTING_CSV_INDEX]);
            $postcode->setNorthing($data[self::NORTHING_CSV_INDEX]);
            $postcode->setLatitude($latLon['lat']);
            $postcode->setLongitude($latLon['lon']);

            $this->em->persist($postcode);
            $transCount++;

            if($transCount > self::INSERT_TRANSACTION_SIZE)
            {
                $this->em->flush();
                $this->em->clear();
                $transCount = 0;
     ...
     * @param int $northing
     *
     * @throws \PHPCoord\Exception\UnknownConversionException
     * @throws \PHPCoord\Exception\UnknownCoordinateReferenceSystemException
     */
    private function convertMetrics(int $easting, int $northing)
    {
        $from = ProjectedPoint::createFromEastingNorthing(
            new Metre($easting),
            new Metre($northing),
            Projected::fromSRID(Projected::EPSG_TM75_IRISH_GRID)
        );
     ...
     * @return mixed
     */
    private function convertToLatLong($north, $east, $utmZone = 31)
    {
